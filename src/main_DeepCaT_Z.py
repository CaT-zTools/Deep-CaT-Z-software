...
# Form implementation generated from reading ui file 'interface_DeepCaT_Z.ui'
#
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.setup_gb.setGeometry(QtCore.QRect(10, 10, 441, 771))
        font = QtGui.QFont()
...
"border-radius:10px;")
        ...
        self.control_gb.setGeometry(QtCore.QRect(470, 10, 681, 771))
...
"")
        ...
        self.label_14.setGeometry(QtCore.QRect(580, 750, 101, 20))
        font = QtGui.QFont()
        font.setPointSize(9)
        font.setBold(False)
        font.setWeight(50)
        ...
        if self.allGood_camera:
            ...
            if self.backsub_model_rb.isChecked():
                ...
                while True:
                    ...
                    if cv2.getWindowProperty('Preview - depth frame: with pre-defined border and dynamic depth range', 1) == -1 :
                        break
                ...
                while True:
                    ...
                    # Close window to abort streaming
                    if cv2.getWindowProperty('Preview - depth frame: with pre-defined dynamic depth range', 1) == -1 :
                        break
    ...
    def timer_start_class_track_function(self):
        ...
        if self.started_timer_track:
        #if self.counter_Totalframes < len(self.files_frames): #& self.started_timer_track:
            ...
            if self.counter_Totalframes < self.N_STEPS -1:
                ...
                self.listLabels.append(-1)
                self.list_centroids.append([-1, -1, -1])
                self.list_ROIs.append(-1)
                self.list_key_pressed.append(-1)
                self.list_outputs_arduino.append(-1)
                ...
                if self.allGood_arduino_connected & self.allGood_arduino_streaming:
                    ...
                    if test_preds_label == 0:
                        state_arduino = 'S'
                    elif test_preds_label == 1:
                        state_arduino = 'W'
                    elif test_preds_label == 2:
                        state_arduino = 'R'
                    elif test_preds_label == 3:
                        state_arduino = 'G'
                    elif test_preds_label == -1:
    ...
    def stop_pb_CLICKED(self):
        ...
        if self.allGood_arduino_connected & self.allGood_arduino_streaming:
            list_outputs_arduino_tosave = np.array(self.list_outputs_arduino).reshape((-1,1))
            ...
            allData_tosave_df = pd.DataFrame (appendList, columns = ['timesteps', 'behavior label', 'centroid x', 'centroid y', 'centroid z', 'ROI id', 'key pressed', 'output arduino'])
            ...
            allData_tosave_df = pd.DataFrame (appendList, columns = ['timesteps', 'behavior label', 'centroid x', 'centroid y', 'centroid z', 'ROI id', 'key pressed'])
